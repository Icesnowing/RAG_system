...
DOCS_DIR = os.environ.get('DOCS_DIR', '../RAG_files')
...
DEFAULT_TEST_SET = os.environ.get('DEFAULT_TEST_SET', 'rag_testsets.csv')
RERANK_MODEL = os.environ.get('RERANK_MODEL', "../models/bge-reranker-base")
...
class LocalRAGSystem:
    ...
    def embeddings(self):
        if self._embeddings is None:
            ...
            model=EMBED_MODEL,
            base_url=OLLAMA_BASE_URL  # 关键：使用环境变量中的URL
            )
...
#===============query预处理=====================
    ...
    def _cross_encoder_rerank(self, query: str, fused_indices: list):
        if self.rerank_model is None:
            return [self.chunks[idx] for idx, _ in fused_indices[:RERANK_TOP_N]]
